...
const AddOne: NextPage<IAddOne> = ({ foods: fds }) => {
  const [name, setName] = useState<string>();
  const [foods, setFoods] = useState<Array<IPostFood>>([]);
  const [choosenFoods, setChoosenFoods] = useState<Array<IFood>>([]);
  const [isActiveMenu, setIsActiveMenu] = useState<Boolean>();
...
export const getServerSideProps: GetServerSideProps = async ({ req, res }) => {
  ...
  if (auth.auth) {
    ...
    return {
      props: { foods }, // will be passed to the page component as props
    };
  } else {
    return {
      redirect: {
